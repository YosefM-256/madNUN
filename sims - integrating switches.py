...
def ic_vce(ib, vtops):
    ib_data = []
    # the program starts with the highest resistor available - improving precision
    set_base_switches("1k") 
    for vtop in vtops:
        set_dac0_bin(vtop)
        tuning_ret = tune_by("ib_dut","dac1", ib, "direct")
        if tuning_ret == "top_breach":
            set_base_switches("100")
            tuning_ret = tune_by("ib_dut","dac1", ib, "direct")
            if tuning_ret == "top_breach":
                raise ValueError('ib ' + str(ib) + " with vtop " + str(vtop) + \
                                 " has caused a top breach")
